...
```
kasa ls [<path>] [--json] [-p PAGE] [-r/--no-recursive]
```

- `<path>` - 記事名/カテゴリ/タグ (省略可)
- `--json` - JSON出力
- `-p` - ページ番号 (default: 1)
- `-r/--no-recursive` - 再帰的にリスト (default: true)
...
```
kasa cp <source> <target> [-f] [--notice/--no-notice] [-p PAGE] [-r/--no-recursive]
```

### rm - Delete posts

```
kasa rm <path> [-s] [-f] [-p PAGE] [-r/--no-recursive]
```
...
```
kasa tag <path> [-t TAG ...] [-o] [-d] [-s] [-f] [--notice/--no-notice] [-p PAGE] [-r/--no-recursive]
```
...
```
kasa wip <path> [-s] [-f] [--notice/--no-notice] [-p PAGE] [-r/--no-recursive]
```

### unwip - Unwip posts

```
kasa unwip <path> [-s] [-f] [--notice/--no-notice] [-p PAGE] [-r/--no-recursive]
```
